...
test("server-renders the task-free course pathway and its full shell", async () => {
  ...
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.match(html, /From your first prompt to an AI system you can run safely/i);
  ...
  assert.match(html, /Context clearing/i);
  assert.match(html, /Models prompting models/i);
