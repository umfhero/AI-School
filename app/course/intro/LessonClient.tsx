...
import { CoursePathDiagram } from "../FoundationDiagrams";
...
export default function IntroLessonClient() {
  ...
  async function completeLesson() {
    ...
    if (!signedIn) return;
    if (lessonComplete || completionStatus === "saving") return;
  ...
  return <main className="lesson-page intro-lesson-page">
    ...
    <div className={`lesson-workspace visual-closed ${sidebarOpen ? "" : "sidebar-closed"}`}>
      ...
      <article className="lesson-reading"><div className="lesson-reading-inner intro-reading model-decision-reading"><p className="reading-kicker">Course introduction</p><h1>Learn to work with AI, without losing the thread.</h1><p className="lesson-lede">AI school is a free course for people who want to use AI to make real things, while staying able to explain what the work is for, what changed and how it was checked. You do not need to code before you begin, although you will see how a project folder and an AI coding tool fit into a useful workflow.</p><div className="lesson-rule" />
        <section><p className="reading-kicker">What this course is</p><h2>A practical course for work that lasts beyond one chat.</h2><p>A chat can help you get a quick answer, a draft or a first version, although a project needs more than that: a clear goal, useful context, decisions that can be found again and a way to check the result before you trust it.</p><p>This course teaches those habits in ordinary language. You will practise directing an AI tool through a piece of work, judging what comes back and deciding what happens next, which is the part of the job that stays yours however capable the tool becomes.</p><CoursePathDiagram /></section>
