...
const tasks: ModelMatchTaskDefinition[] = [
  { taskId: "pace", number: "Task 01", title: "Match the pace", instruction: "Select a task on the left, then the most sensible starting priority on the right.", cardTitle: "TASK 01 · MATCH THE PACE", cardPrompt: "Connect each piece of work to a sensible speed and reasoning priority.", pairs: [
    ...
    { left: "Review a safety-critical calculation", right: "Prioritise careful reasoning and checking" },
  ] },
...
export default function SpeedCostReasoningClient() {
  return <ModelLessonShell lessonId="models/speed-cost-reasoning" lessonNumber="02.2" title="Speed, cost and reasoning" lede="A quick answer can be exactly what a small task needs, while difficult or high-consequence work can justify more time, more reasoning effort and a larger bill. The useful choice comes from the work and the cost of being wrong, rather than the model with the biggest score." currentPath="/course/models/speed-cost-reasoning" previousHref="/course/models/what-changes" nextHref="/course/models/context-windows" completionTitle="You can match model effort to the consequence of the work." tasks={tasks}>
    {({ completedTasks, openTask }) => <>
      <section><p className="reading-kicker">Section 1</p><h2>Speed has more than one part.</h2><p>Some models begin writing almost at once, while others pause before the first word because they are processing a larger request or using extra reasoning. Once the answer begins, output speed describes how quickly more text arrives, so two models can feel different even when their full answers take a similar amount of time.</p><p>A fast response matters during a live call, when drafting many short labels, or when the learner needs quick feedback. A slower response can be reasonable when the task has dependent calculations or several rules that need to be checked together, although waiting longer does not prove that the answer is correct.</p><LessonPhoto src="/lesson-images/model-training-cost.jpg" alt="Bar chart of estimated training costs for selected AI models from 2017 to 2023" eyebrow="TRAINING COST HAS RISEN" title="Training a model can require substantial computing. That helps explain why capability, response pace and price can move together without following one simple rule." sourceHref="https://commons.wikimedia.org/wiki/File:Estimated_training_cost_of_some_AI_models_-_2024_AI_index.jpg" sourceLabel="Stanford AI Index on Wikimedia Commons" contain /><SpeedReasoningDiagram /><ModelLessonTaskCard task={tasks[0]} completedTasks={completedTasks} openTask={openTask} /></section>
      ...
      <section><p className="reading-kicker">Section 3</p><h2>Spend according to the consequence.</h2><p>The lowest price matters when thousands of simple, repeatable requests are being processed, but the cheapest model can become expensive if its answers need several retries or hours of repair. A stronger model can also waste money when the job is a disposable brainstorm that a faster model already handles well.</p><p>Start with the consequence of an incorrect answer, because this decides how much model effort and checking the task deserves. A rough heading can be replaced in seconds, while a missed legal deadline, an incorrect medical claim or a broken customer refund needs source checking and a responsible person to make the final decision. More model reasoning can support that process, while the responsible person still stays in control.</p><div className="consequence-ladder"><div><span>LOW</span><b>Easy to undo</b><p>Use a quick, low cost first pass and check the visible result.</p></div><div><span>MEDIUM</span><b>Affects project work</b><p>Use a capable model, compare the answer with the brief, and record useful decisions.</p></div><div><span>HIGH</span><b>Hard to undo</b><p>Use stronger reasoning, check primary sources, and keep a qualified person in control.</p></div></div><ModelLessonTaskCard task={tasks[2]} completedTasks={completedTasks} openTask={openTask} /></section>
    </>}
  </ModelLessonShell>;
}
