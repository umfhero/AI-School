...
export async function GET(request: Request) {
  try {
    ...
    const body = JSON.stringify({ exportedAt: new Date().toISOString(), account: { email: user.email, name: user.name, pictureUrl: user.pictureUrl }, progress: progress.results ?? [] }, null, 2);
