...
## Rebuilt course status

- The public curriculum currently contains one chapter and one lesson, `/course/chapter-1/lesson-1`. This task-free orientation introduces the ten-chapter route recorded in `aicourse.md`, then lets the learner complete it without answering a task.
...
- Keep the shared course contents sidebar in `LessonTemplate`. It uses the homepage's ink borders, white and lilac surfaces, violet accents, and changes completed chapters and lessons to the shared green state. Pastel yellow is reserved for a small focal accent and is not a sidebar surface. Use the existing pixel icons or CSS pixel geometry for status controls rather than text glyphs.
