...
- **Concept diagrams:** `LessonDiagram` is the shared frame for original SVG teaching diagrams. It supplies the hard border, compact diagram header, dark canvas and explanatory caption. The lesson route supplies the SVG itself. Every concept lesson needs at least one SVG diagram, while sourced figures remain useful when the original image is evidence. Build diagrams from crisp rectangles, right-angle paths, flat fills and hard offset shadows, with `shapeRendering="crispEdges"` where it helps. Wide diagrams should switch to a purpose-built mobile arrangement below 620px. Animated markers must use square pixels, move through a dedicated connector rail rather than over cards or text, explain a path or state and hide when reduced motion is requested. Provide a hidden text equivalent when the SVG contains lesson information that does not appear elsewhere on the page.
