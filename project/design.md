...
- **Pointer feedback:** `LessonPointerEffects` replaces the native pointer with a pixel arrow, changes it to a pixel hand over interactive elements and leaves a short hand-shaped click trace. Touch devices, reduced motion users and browsers without fine pointer support keep the native experience. Custom pointer styling is scoped to `.lesson-page` so it does not leak into the rest of the site.
