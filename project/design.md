...
- **Task placement:** quick multiple choice, sorting and left to right matching tasks stay inside the reading column. Use `LessonTaskCard` and `LessonTaskPanel` when the learner needs more room while keeping the lesson visible beside the work. On desktop, the panel becomes a third workspace column. At smaller widths it becomes a bottom sheet. Keep the close button, Escape key behaviour, focus transfer, answer state and feedback region. After completion, the card must keep a green completed state and reopen with the completed answer intact.
...
- **Reusable icons:** the task panel close control uses `PixelClose` from `app/components/PixelIcons.tsx`. Add new shared pixel shapes there rather than drawing a lesson-specific copy.

## Rollout

Applied so far: the homepage, `/profile` and the starter lesson at `/course/chapter-1/lesson-1`. The starter lesson now defines the reading, image, task and pointer patterns for the new course. Future lessons should reuse these patterns and the existing palette rather than introduce another lesson shell.
