...
Applied so far: the homepage, `/profile` and the course pathway at `/course/chapter-1/lesson-1`. The first lesson now defines the reading, SVG diagram, task-free completion and pointer patterns for the new course. Future lessons should reuse these patterns and the existing palette rather than introduce another lesson shell.
