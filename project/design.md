...
| course pastel yellow    | `#fbf5ab`               | One focal course-map card or small accent, used sparingly   |
| course violet           | `#6650e6`               | Course map rail, active navigation and hard accent shadows  |
...
Hand-authored inline SVG components in `app/components/PixelIcons.tsx` (`PixelArrow`, `PixelSpark`, `PixelCheck`, `PixelCursor`, `PixelPointer`, `PixelMascot`), built as a grid of `<rect>` elements on a small `viewBox` (7×7 to 10×13), rendered with `shapeRendering="crispEdges"` so they stay blocky rather than antialiased. Import from that shared file rather than redefining an icon locally. `PixelArrow` points right by default; rotate it with a CSS `transform` (e.g. `rotate(-90deg)` for up, `rotate(180deg)` for left) rather than drawing a new direction:
...
`/course/chapter-1/lesson-1` is the reference shell for the rebuilt course. New lessons should reuse its structure and shared components rather than copying a full lesson and changing it by hand.

- **Reading width:** `.lesson-template-reading` gives desktop lessons a wider measure while keeping paragraph lines readable. It starts with the first numbered section. Do not add a separate lesson title, introductory rule or decorative opening block above it.
- **Section copy:** section paragraphs use the shared larger text size and blue-grey colour. The numbered kicker remains small and violet, so it works as navigation rather than competing with the heading.
- **Sourced figures:** `.lesson-image-template` keeps an image, teaching caption and source link together. Use a relevant graph, model diagram or training image, explain what the learner should notice and link to the original publisher.
- **Concept diagrams:** `LessonDiagram` is the shared frame for original SVG teaching diagrams. Its default tone supplies the hard border, compact diagram header, dark canvas and explanatory caption. Use the paper tone when the artwork needs to sit directly on the lesson page, with a transparent canvas, light lilac header and separate white caption. The paper header does not use an offset shadow. The lesson route supplies the SVG itself. Every concept lesson needs at least one SVG diagram, while sourced figures remain useful when the original image is evidence. Build diagrams from crisp rectangles, right-angle paths and flat fills, with `shapeRendering="crispEdges"` where it helps. Do not place an offset panel behind every SVG card. A diagram header may include a specific fact such as a chapter count, but it must not label itself as an animated SVG. Wide diagrams should switch to a purpose-built mobile arrangement below 620px. Animated markers must use square pixels, move through a dedicated connector rail rather than over cards or text, explain a path or state and hide when reduced motion is requested. Provide a hidden text equivalent when the SVG contains lesson information that does not appear elsewhere on the page.
- **Course contents:** the shared sidebar uses a white heading area, violet progress, lilac states and white chapter controls with ink borders. Pastel yellow is not a sidebar surface. The current chapter and lesson use violet hard shadows, while completed items switch to the shared green success treatment. Reuse `PixelCheck` and CSS pixel marks instead of Unicode status symbols.
- **Inline tasks:** multiple choice, sorting and left to right matching tasks stay inside the reading column through `InlineLessonTask`. Do not open lesson tasks in a side panel or bottom sheet. Keep the question, choices, feedback, progress and check action together, then change the whole task block to its green completed state after the correct answer.
- **Pointer feedback:** `SitePointerEffects` runs from the root layout, so the pixel pointer covers every page. It replaces the native cursor with a single pixel arrow sprite that never changes on hover, and a left click spawns a separate short burst at the pointer: four flat violet and mint squares fly out in a cross while an ink dot flashes at the centre, then everything fades in steps. The sprite lives in `PixelIcons.tsx` and is drawn with an ink outline, a white face and a hard ink offset shadow, matching the card shadow formula. Touch devices, reduced motion users and browsers without fine pointer support keep the native cursor. Custom pointer styling is gated on a body class so it can be disabled without touching page styles.
- **Reusable icons:** task controls use the shared arrow and check icons from `app/components/PixelIcons.tsx`. Add new shared pixel shapes there rather than drawing a lesson-specific copy.

## Rollout

Applied so far: the homepage, `/profile` and the course pathway at `/course/chapter-1/lesson-1`. The first lesson now defines the reading, paper-tone pixel SVG, task-free completion, restrained course palette, shared course sidebar and pixel pointer patterns for the new course. Its pathway animation stays in a separate rail and the mobile layout is authored independently, so future diagrams should use the same separation rather than shrink or layer desktop artwork over content.
